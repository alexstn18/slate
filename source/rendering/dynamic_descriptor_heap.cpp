	...
	, m_NumFreeHandles{ 0u }
{
	m_DescriptorHandleIncrementSize = 
		App.Renderer().D3D12Device()->GetDescriptorHandleIncrementSize( type );

	m_DescriptorHandleCache = std::make_unique<D3D12_CPU_DESCRIPTOR_HANDLE[]>(
	...
	const D3D12_CPU_DESCRIPTOR_HANDLE srcDescriptor)
{
	...
	if ( numDescriptors > m_NumDescriptorsPerHeap || 
		rootParameterIndex >= MaxDescriptorTables ) {
		throw std::bad_alloc();
	}

	DescriptorTableCache& descriptorTableCache = 
		m_DescriptorTableCache[ rootParameterIndex ];
	...
	D3D12_CPU_DESCRIPTOR_HANDLE* dstDescriptor = 
		( descriptorTableCache.BaseDescriptor + offset );
	for ( u32 i{ 0u }; i < numDescriptors; ++i ) {
		dstDescriptor[i] = CD3DX12_CPU_DESCRIPTOR_HANDLE(
			srcDescriptor, i, m_DescriptorHandleIncrementSize
		);
	}
	...
	setFunc)
{
	...
	if ( numDescriptorsToCommit > 0u ) {
		...
		if ( !m_CurrentDescriptorHeap || m_NumFreeHandles < numDescriptorsToCommit ) {
			...
			commandList.Get()->SetDescriptorHeaps( 1u, heaps );
		...
		while ( _BitScanForward( &rootIndex, m_StaleDescriptorTableBitMask ) ) {
			...
			D3D12_CPU_DESCRIPTOR_HANDLE* pSrcDescriptorHandles = 
				m_DescriptorTableCache[ rootIndex ].BaseDescriptor;
			...
			m_CurrentCPUDescriptorHandle.Offset(
				numSrcDescriptors, m_DescriptorHandleIncrementSize
			);
			m_CurrentGPUDescriptorHandle.Offset(
				numSrcDescriptors, m_DescriptorHandleIncrementSize
			);
...
{
	if ( !m_CurrentDescriptorHeap || m_NumFreeHandles < 1u ) {
		m_CurrentDescriptorHeap = RequestDescriptorHeap();
	...
	device->CopyDescriptorsSimple(
		1u, m_CurrentCPUDescriptorHandle, cpuDescriptor, m_DescriptorHeapType
	);

	m_CurrentCPUDescriptorHandle.Offset( 1u, m_DescriptorHandleIncrementSize );
	m_CurrentGPUDescriptorHandle.Offset( 1u, m_DescriptorHandleIncrementSize );
	m_NumFreeHandles -= 1u;
...
void DynamicDescriptorHeap::ParseRootSignature(const RootSignature& rootSignature)
{
	...
	while ( _BitScanForward( &rootIndex, descriptorTableBitMask ) &&
		...
		descriptorTableCache.BaseDescriptor = 
			m_DescriptorHandleCache.get() + currentOffset;
...
void DynamicDescriptorHeap::Reset()
{
	...
	for ( u32 i{ 0u }; i < MaxDescriptorTables; ++i ) {
