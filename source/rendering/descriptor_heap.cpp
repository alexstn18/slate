...
void DescriptorHeap::Initialize(HeapType type, u32 maxDescriptors)
{
	...
	m_CurrentIndex = 1u;
	log::Info(
		"Created descriptor heap: type={}, descriptors={}, size={}", 
		HeapTypeAsString( type ), maxDescriptors, m_DescriptorSize
	);
}
...
{
	log::Assert( 
		m_CurrentIndex < m_MaxIndex, "Descriptor heap is full ({}/{})",
...
CD3DX12_CPU_DESCRIPTOR_HANDLE DescriptorHeap::GetCPUHandle(uint32_t index)
{
	log::Assert(
		index < m_MaxIndex, 
		"Descriptor index {} out of bounds (max: {})", 
		index, m_MaxIndex
	);
	return CD3DX12_CPU_DESCRIPTOR_HANDLE(
		m_Heap->GetCPUDescriptorHandleForHeapStart(), i32( index ), m_DescriptorSize
	);
}
...
{
	log::Assert(
		m_Heap->GetDesc().Flags & D3D12_DESCRIPTOR_HEAP_FLAG_SHADER_VISIBLE,
		"Heap must be shader visible to get GPU handle"
	);

	return CD3DX12_GPU_DESCRIPTOR_HANDLE(
		m_Heap->GetGPUDescriptorHandleForHeapStart(), i32( index ), m_DescriptorSize
	);
}
